...
enum dungeonFeatureTypes {
    ...
    // an insight altar promotes to its inert form after a sacrifice
...
enum terrainMechanicalFlagCatalog {
    ...
    TM_INSIGHT_ACTIVATION           = Fl(26),       // in machine, when the insight + offering altars both hold items, reveal the insight item and consume the offering, then activate
...
enum machineTypes {
    ...
    // Brogue fills this variant-specific reward slot with the altars of insight (force-built at fixed
    // depths in addMachines; never collides with the BulletBrogue weapon vault, which occupies the same
    // index only in BulletBrogue's catalog).

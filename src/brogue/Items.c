...
// a fire/lightning bolt crossing a dropped bad potion detonates it (helper defined near throwItem)
...
static boolean updateBolt(bolt *theBolt, creature *caster, short x, short y,
    ...
    // A fire or lightning bolt crossing a DROPPED bad potion detonates it in place, spawning the
    // potion's normal shatter signature. Placed before the BF_FIERY exposeTileToFire below so a fire
    // bolt ignites the freshly-spawned flammable gas (violent), while lightning leaves it as a gas
...
// Spawn a bad/cloud potion's shatter signature at (x,y), auto-identify the flask, refresh the cell.
// Returns true for the 7 cloud/explosion kinds, false otherwise. Shared by throwItem and the
// bolt-detonation hook in updateBolt so the kind->DF mapping lives in one place.
static boolean shatterPotionAtLoc(item *theItem, short x, short y) {
    switch (theItem->kind) {
        case POTION_POISON:
...
//          }
    ...
    if ((theItem->category & POTION) && (hitSomethingSolid || !cellHasTerrainFlag((pos){ x, y }, T_AUTO_DESCENT))) {
        // Bad/cloud-potion shatter extracted to shatterPotionAtLoc (shared with the bolt-detonation
        // hook in updateBolt). Returns true (signature spawned + flask auto-identified) for the 7
        // cloud/explosion kinds; otherwise falls through to the harmless-splash branch.
